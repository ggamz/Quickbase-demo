...
{
    public class ConcreteStatDataProvider : ICountryDataProvider
    {
        public async Task<List<Country>> GetCountries()
        {
			var data = await GetCountryPopulationsAsync();
            if (data == null)
            {
				return null;
            }
			return data.Select(t => new Country(t.Item1, t.Item2)).ToList();
		}
